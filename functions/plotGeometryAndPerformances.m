...
function plotGeometryAndPerformances(x_norm_final, mission, prop, motor, env, opt)
    ...
    t.Units = 'normalized';
    t.Position(2) = t.Position(2) + 0.055;   % increase vertical offset
    % Don't make the offset smaller than 0.055 or the title will overlap the figure.
    ...
    figure('Name', '9. Bending Stress (at the critical point of each section) distribution', 'Position', [100 100 1500 600]);
    ...
    sgtitle('\textbf{9. Bending Stress (at the critical point of each section) distribution}', 'Interpreter', 'latex');
    
    
    % --- Total stresses graph (\sigma_{total max}) ---
    figure('Name', '10. Total Stress (at the critical point of each section) distribution', 'Position', [100 100 1500 600]);
    ...
    sgtitle('\textbf{10. Total Stress (at the critical point of each section) distribution}', 'Interpreter', 'latex');
    ...
    annotation('textbox', [0.03, 0.88, 0.25, 0.1], ...
        'String', note_str, ...
        'Interpreter', 'latex', ...
        'FontSize', 7, ...
        'Color', [0.3 0.3 0.3], ... 
    ...
    mesh = nonEmptyOutput.mesh;
    dx   = nonEmptyOutput.dx;

    % Create the figure
    figure('Name', '11. Airfoil Stress Distribution (at Critical Section)', ...
    ...
    for k = 1:num_segments
        ...
        % --- B. Identify Critical Section (CONSIDERING THAT IT HAS TO BE AT X >= X_FINISH_REINFORCEMENT) ---
        % Filter: Only consider sections outside the reinforcement zone
        valid_region_mask = out.mesh >= prop.x_finish_reinforcement;

        % Create a temporary array for finding the max
        search_stress = out.sigma_total_max;
        ...
        % Store for plotting
        plot_data{k}.idx_crit = idx_crit; % This is the critical section index. It is NOT the critical point inside section index (jcrit)
        plot_data{k}.sigma_pts = out.sigma_total_pts(:, idx_crit); % idx_crit is the critical section index (column)
        plot_data{k}.M_x = out.M_x(idx_crit);
        plot_data{k}.M_y = out.M_y(idx_crit);
        plot_data{k}.c   = out.c(idx_crit);
        plot_data{k}.theta = out.theta(idx_crit);
        plot_data{k}.r_R   = x_critical;
    ...
    % 1. Format the number nicely (e.g., 2 decimal places)
    x_val_str = num2str(prop.x_finish_reinforcement, '%.2f'); 
    
    % 2. Define the lines as a Cell Array
    % Line 1: Main Title (Bold)
    line1 = '\textbf{11. Airfoil Stress Distribution at critical section for each mission segment}';
    
    % Line 2: Subtitle (Normal weight, Italicized, with Math logic)
    % We concatenate [...] the strings to insert the variable x_val_str
    line2 = ['\textnormal{\textit{Note: Only the aerodynamic region of the blade is being considered ($x \ge ' x_val_str '$)}}'];
    
    % 3. Create the annotation
    annotation('textbox', [0, 0.88, 1, 0.10], ...  % Increased height slightly to fit 2 lines
        'String', {line1; line2}, ...              % <--- Use Cell Array { ; } for line breaks
        'Interpreter', 'latex', ...
