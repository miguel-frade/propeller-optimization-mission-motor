...
function plotGeometryAndPerformances(x_norm_final, mission, prop, motor, env, opt)
    ...
    % Predefine the style for the indication of where the structural
    % reinforcement finishes. We should only focus on the stresses to the
    % right of this prop.x_finish_reinforcement (those are the stresses to
    % which we apply the structural restriction).
    ...
    reinfLabel = { ...
        'End of structural reinforcement', ...
        'Focus only on stresses to the right' ...
    };
    ...
    for i = 1:num_segments
        ...
        hold on
        xline(x_reinf_end, reinfLineStyle{:}, reinfLabel, ...
              'LabelVerticalAlignment', 'bottom', ...   % keeps label away from high stresses
    ...
    for i = 1:num_segments
        subplot(1, num_segments, i);
        plot(outputs{i}.mesh, outputs{i}.sigma_bend_max_Ix / 1e6, 'b', 'LineWidth', 2, 'DisplayName', '\sigma_{bend Ix max}')
        hold on
        plot(outputs{i}.mesh, outputs{i}.sigma_bend_max_Iy / 1e6, 'r--', 'LineWidth', 2, 'DisplayName', '\sigma_{bend Iy max}')
        xline(x_reinf_end, reinfLineStyle{:}, reinfLabel, ...
              'LabelVerticalAlignment', 'bottom', ...   % keeps label away from high stresses
              'LabelHorizontalAlignment', 'left', ...
              'Interpreter', 'latex', ...
        ...
        legend('\sigma_{bend Ix max}','\sigma_{bend Iy max}', 'Location', 'best')
        grid on
        title(titles{i}, 'Interpreter', 'latex', 'FontSize', 14);
    ...
    % 4. Loop to Resize (Maximize) and Save - WHILE INVISIBLE
    %    Open all figures on maximized windows
    for i = 1:length(allFigs)
        hFig = allFigs(i);
        
        % A. Force the figure to occupy 100% of the screen space
        % This simulates "Maximized" for the saved file without showing the window.
        set(hFig, 'Units', 'normalized', 'OuterPosition', [0 0 1 1]);
        
        % B. Force MATLAB to recalculate layouts (legends, titles) for the new size
        drawnow;
        
        % C. Construct Filename
        fName = hFig.Name;
        if isempty(fName), fName = sprintf('Figure_%d', hFig.Number); end
        % Sanitize filename (replace spaces with _, remove symbols)
        safeName = regexprep(strrep(fName, ' ', '_'), '[^a-zA-Z0-9_]', '');
        fullPath = fullfile('results', [safeName, '.jpg']);
        
        % D. Save
        saveas(hFig, fullPath, 'jpeg');
        fprintf('Saved: %s\n', fullPath);
    end
    
    % 5. Finally, Restore Visibility and Show Windows Maximized
    % We restore the default setting first
    set(0, 'DefaultFigureVisible', 'on');
    
    for i = length(allFigs):-1:1 % Reverse order so Fig 1 ends up on top
        hFig = allFigs(i);
        figure(hFig);             % Bring to focus
        set(hFig, 'Visible', 'on'); 
        try
            hFig.WindowState = 'maximized'; % Actual screen maximization
        catch
