...
if prop.x0<1e-6, warning('x0 can not be zero for SABEMMT'); end
...
prop.x_finish_reinforcement = 0.15; % This is a realistic value (do not
                                    % make it smaller).
...
% Mesh Setup
prop.n = 100;
prop.mesh = linspace(prop.x0, 1, prop.n); % Non-dimensional. ROW vector.
prop.dx = gradient(prop.mesh); % Non-dimensional
